...
class Auth(GridLayout):
    ...
    def check_token(self, parent_layout, left_layout):
        ...
        server_url = 'https://fastapi-1dyl.onrender.com/api/v1/messages/health'
        if not self.is_server_available(server_url):
            # Если сервер недоступен, показываем сообщение
            message = 'Сервер временно недоступен. Попробуйте позднее!'
            notification_manager.show_popup_error(message)
            return
    ...
    def is_server_available(self, url, timeout=5):
        try:
...
class Courses(GridLayout):
    ...
    def on_courses_success(self, request, result, parent_layout, left_layout):
        ...
        parent_layout.clear_widgets()
        left_layout.clear_widgets()
        courses_name_layout = BoxLayout(orientation='vertical', spacing=10, padding=[10, 10, 10, 10])  # Добавление отступов и промежутка
        ...
        for course in result:
            ...
            courses_name_layout.add_widget(Widget(size_hint_y=None, height=5))
...
class Groups(GridLayout):
    ...
    async def load_chat_history(self, temp_id_val):
        ...
        token = App.get_running_app().token
        url = f'https://fastapi-1dyl.onrender.com/chat/{temp_id_val}'
        headers = {'Authorization': f'Bearer {token}'}
    ...
    async def websocket_connect(self, chat_text_layout):
        uri = f"wss://fastapi-1dyl.onrender.com/chat/{self.chat_id}"
...
class MyApp(App):
    ...
    is_image_bg = BooleanProperty(True)  # логическое состояние для отслеживания текущего фона
    ...
    def build(self):
        ...
        self.screen_change_button = Button(text='Фон', background_normal='gray.png', background_down='green.png', color=(0, 0, 0, 1))
        self.screen_change_button.bind(on_press=self.screen_change)
        self.nav_gridlayout_onbase.add_widget(self.screen_change_button)
    ...
    def screen_change(self, instance):
        self.is_image_bg = not self.is_image_bg
        with self.base_gridlayout.canvas.before:
            self.base_gridlayout.canvas.before.clear()
            if self.is_image_bg:
                self.central_bg = Rectangle(source='1.png', pos=self.base_gridlayout.pos,
                                            size=self.base_gridlayout.size)
            else:
                # Color(0.75, 0.75, 0.75, 1)  # Серебристый металик
                Color(0, 0, 0.25, 1)
                self.central_bg = Rectangle(pos=self.base_gridlayout.pos,
                                            size=self.base_gridlayout.size)
            Color(0, 0, 1, 1)  # Синий цвет для границы
            self.central_rect = Line(rectangle=(
            self.base_gridlayout.x, self.base_gridlayout.y, self.base_gridlayout.width,
